...
public class TechnicalRecyclerAdapterOnlineDanger extends RecyclerView.Adapter<TechnicalRecyclerAdapterOnlineDanger.TechnicalViewHolder> {

    private List<CustomerOnline> list;
    private TechnicalActivityOnline technicalActivity;
    MediaPlayer mp;
    public TechnicalRecyclerAdapterOnlineDanger(List<CustomerOnline> list, TechnicalActivityOnline technicalActivity) {
        this.list = list;
    ...
    public void onBindViewHolder(@NonNull final TechnicalViewHolder holder, @SuppressLint("RecyclerView") final int i) {
        ...
        holder.TecLinear.setBackground(technicalActivity.getResources().getDrawable(R.drawable.shadow_danger));
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            @RequiresApi(api = Build.VERSION_CODES.M)
            @Override
            public void onClick(View v) {
                technicalActivity.getLocation(list.get(i),3);
               try {
                   mp.stop();
                   mp.release();
               }catch (Exception e){
